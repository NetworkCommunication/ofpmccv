...
class ReplayBuffer:
    def __init__(self, capacity=1e5, seed=0):
...
class NStepReplayBuffer(ReplayBuffer):
    ...
    def __init__(self, capacity=1e5, seed=0):
        super(NStepReplayBuffer, self).__init__(capacity, seed)
        pass

    def sample(self, batch_size):
        batch = random.sample(self.buffer, batch_size)
        state, action, reward, next_state, done, n_step_return = zip(*batch)
        return state, action, reward, next_state, done, n_step_return

    def append(self, state, action_with_param, reward, next_state, done, n_step_return):
        if len(self.buffer) < self.capacity:
            self.buffer.append(None)
        self.buffer[self.position] = (state, action_with_param, reward, next_state, done, n_step_return)
        self.position = int((self.position + 1) % self.capacity)
