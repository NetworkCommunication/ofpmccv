...
# Whether to open the GUI interface
gui = False
if gui:
...
class LaneChangePredict(gym.Env):
    # Defines the metadata of the gym environment
    ...
    def __init__(self):
        ...
        self.maxLaneNumber = 2     # Number of lanes

        self.CommRange = 150   # joint sensing range

        self.delta_t = 0.2
        self.AutoCarID = 'Car'   # CV
        self.PrevSpeed = 0
        ...
        self.overpassFlag = 0
        self.AutoCarFrontID = 'CarF'  # TV
        self.ttc_safe = 3  # Minimum safe TTC value

        self.action_space_vehicle =[-1, 0, 1]   # 0: keep lane, -1: Change lane to the left, 1: Change lane to the right
    ...
    def reset(self):
        self.TotalReward = 0
        self.numberOfLaneChanges = 0   # Number of times the vehicle changed lanes
        self.numberOfOvertakes = 0   # Number of overtakes completed
    ...
    # Find and return the action array of the corresponding row
    ...
    def step(self, action, action_param):
        ...
        self.PrevSpeed = Vehicle_Params[self.AutoCarID][tc.VAR_SPEED]   # Get the speed of the vehicle before execution
        self.PrevVehDistance = Vehicle_Params[self.AutoCarID][tc.VAR_LANEPOSITION]  # Get the longitudinal position of the vehicle's lane before execution
    ...
    def _findstate(self):
        # Get the state list of subscribed vehicles
    ...
    # Process the reward function so that its various parameter ranges are similar
    ...
    def updateReward(self, action, state):
        ...
        if isOC != 1:
            ...
            df.to_csv('result/normal/numberOfLaneChanges0820.csv', index=False, mode='a', header=False)
