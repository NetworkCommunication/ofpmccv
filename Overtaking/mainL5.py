...
from my_EnvL5 import LaneChangePredict
import numpy as np
...
def run(seed, episodes, batch_size, gamma, inverting_gradients, initial_memory_threshold, replay_memory_size,
    ...
    for i_eps in range(episodes):
        ...
        # Reset an episode's information, status, and actions
        info = {'status': "NOT_SET"}
        state = env.reset()
        ...
        for i_step in range(max_steps):
            ...
            # Save last episode information
            if i_eps == episodes - 1:
                df1 = pd.DataFrame({'speedControl': (np.tanh(act_param.cpu().numpy()) + 1) * 10})
                ttc = env.getFinaTCC()
                df2 = pd.DataFrame({'TCC': ttc},index=[i_step])
                df1.to_csv('result/other/speedControl.csv', index=False, mode='a', header=False)
                df2.to_csv('result/other/TTC.csv', index=False, mode='a', header=False)
    ...
    df.to_csv('result/other/reward.csv', index=False, header=False)
