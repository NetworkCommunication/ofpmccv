...
def run(seed, episodes, batch_size, gamma, inverting_gradients, initial_memory_threshold, replay_memory_size,
    ...
    for i_eps in range(episodes):
        ...
        for i_step in range(max_steps):
            ...
            if i_eps == episodes - 1:
                df1 = pd.DataFrame({'speedControl': (np.tanh(act_param) + 1) * 10},index=[i_step])
                ttc = env.getFinaTCC()
                df2 = pd.DataFrame({'TCC': ttc},index=[i_step])
                df1.to_csv('result/main/speedControl.csv', index=False, mode='a', header=False)
                df2.to_csv('result/main/TTC.csv', index=False, mode='a', header=False)
    ...
    df.to_csv('result/main/reward.csv', index=False, header=False)
