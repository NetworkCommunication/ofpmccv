...
from my_EnvDefensive import LaneChangePredict
import numpy as np
...
@click.option('--inverting-gradients', default=True,
              help='Use inverting gradients scheme instead o`f squashing function.', type=bool)
@click.option('--initial-memory-threshold', default=1000, help='Number of transitions required to start learning.',
              type=int)
@click.option('--replay-memory-size', default=6000, help='Replay memory size in transitions.', type=int)  # 500000
...
def run(seed, episodes, batch_size, gamma, inverting_gradients, initial_memory_threshold, replay_memory_size,
    ...
    for i_eps in range(episodes):
        ...
        for i_step in range(max_steps):
            ...
            if i_eps == episodes - 1:
                ...
                df1.to_csv('result/defensive/speedControl.csv', index=False, mode='a', header=False)
                df2.to_csv('result/defensive/TTC.csv', index=False, mode='a', header=False)
    ...
    df.to_csv('result/defensive/reward0820.csv', index=False, header=False)
...
if __name__ == '__main__':
    ...
    sumocfg_file = "defensive/StraightRoad.sumocfg"
